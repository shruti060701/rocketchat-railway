...
# (Rocket.Chat) can actually resolve, not localhost, since localhost only
# works for clients inside this same container. MONGODB_ADVERTISED_HOSTNAME
# is set to this service's Railway private-network domain for that reason.

: "${MONGODB_ADVERTISED_HOSTNAME:?MONGODB_ADVERTISED_HOSTNAME must be set to this own service RAILWAY_PRIVATE_DOMAIN value}"
...
if [ "$STATUS" != "1" ]; then
  ...
  mongosh <<EOF
rs.initiate({_id: "rs0", members: [{ _id: 0, host: "$MONGODB_ADVERTISED_HOSTNAME:27017" }]})
EOF
else
